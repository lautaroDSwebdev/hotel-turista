...
export function Footer() {
  ...
  const { NAV_LINKS } = info.data;

  const SOCIALS = [
    { label: "Twitter", href: "https://twitter.com", Icon: TwitterIcon },
    {
      label: "Facebook",
      href: "https://www.facebook.com/HotelTuristaTandil/",
      Icon: FacebookIcon,
    },
    ...
    <footer className="border-t border-border bg-secondary">
      <div className="mx-auto grid max-w-7xl gap-10 px-6 py-16 md:grid-cols-3">
        <div>
          ...
          <p className="mt-5 max-w-xs text-pretty text-sm leading-relaxed text-muted-foreground">
            Un alojamiento cálido y confortable en el corazón de Tandil,
            ofreciendo la mejor atención y hospitalidad para que te sientas como
            en casa.
          </p>
        </div>
      ...
      <div className="border-t border-border">
        <p className="mx-auto max-w-7xl px-6 py-6 text-center text-xs text-muted-foreground">
          © {new Date().getFullYear()} Hotel Turista Tandil. Todos los derechos
          reservados.
        </p>
      </div>
    </footer>
